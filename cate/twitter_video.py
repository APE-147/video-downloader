...
Cookies can be pre-generated by rss-inbox and injected via ``--cookie-json``.
Inline strings or cookies.txt files merge on top of each other, with later
...
Options::
    ...
    --cookie-file PATH         Netscape-style cookies.txt file or simple
                               ``KEY=VALUE`` lines.
    --cookie-json PATH         JSON file containing ``[{"name": ..., "value": ...}, ...]``.

Environment variables::

    TWITTER_COOKIE             Same as ``--cookie``
    TWITTER_COOKIE_FILE        Same as ``--cookie-file``
    TWITTER_COOKIE_JSON        Same as ``--cookie-json``
"""
...
def load_cookie_json(path: str) -> Dict[str, str]:
    try:
        ...
        raise FileNotFoundError(f"Cookie JSON not found: {path}") from exc
    except json.JSONDecodeError as exc:
        raise ValueError(f"Invalid cookie JSON in {path}: {exc}") from exc

    cookies: Dict[str, str] = {}

    def _ingest_collection(items: Iterable[object]) -> None:
        for item in items:
...
def resolve_cookies(args: argparse.Namespace) -> Optional[Dict[str, str]]:
    inline_cookie = getattr(args, "cookie", None) or os.environ.get("TWITTER_COOKIE")
    cookie_file = getattr(args, "cookie_file", None) or os.environ.get("TWITTER_COOKIE_FILE")
    cookie_json = getattr(args, "cookie_json", None) or os.environ.get("TWITTER_COOKIE_JSON")
    ...
    json_based = load_cookie_json(cookie_json) if cookie_json else None

    return build_cookie_jar(json_based, file_based, inline)
